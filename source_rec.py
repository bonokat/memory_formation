...
if __name__ == '__main__':
    ...
    parser.add_argument('-bl_start', '--start of the baseline interval', type=float, help='set start of baseline interval', default=3.25)
    parser.add_argument('-bl_fin', '--finish of the baseline interval', type=float, help='set finish of baseline interval', default=3.75)
    ...
    bl = tuple((bl_start, bl_fin)) # if bl is not None else tuple()
    ...
    beta = (15., 30.)
    gamma = (30., 60.)
    freq_bands = [alpha, beta, gamma] # [theta, alpha, beta, gamma]
    ...
    for sub_ind, subject_name in zip(sub_inds, os.listdir(subjects_dir)):
        ...
        for epochs_file in os.listdir(epochs_dir):
            ...
            for freq_band in freq_bands:
                ...
                # # set frequency bands to epochs
                # epochs_theta = epochs.copy().filter(theta)
                # epochs_alpha = epochs.copy().filter(alpha)
                # epochs_beta = epochs.copy().filter(beta)
                # epochs_gamma = epochs.copy().filter(gamma)
                ...
                stc_hits_path = os.path.join(source_dir, f'stc_hits_{freq_band}_{subject_name}')
                stc_miss_path = os.path.join(source_dir, f'stc_miss_{freq_band}_{subject_name}')
                stc_diff_path = os.path.join(source_dir, f'stc_diff_{freq_band}_{subject_name}')
